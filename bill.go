...
func generateBill() {
	...
	fmt.Println()
	fmt.Printf("+%s+\n", strings.Repeat("-", 90))
	fmt.Printf(" %-30s %-20s %-20s %-20s\n", "Item Name", "Price(RWF)", "Quantity", "Total Price(RWF)")
	fmt.Printf("+%s+\n", strings.Repeat("-", 90))
	...
	fmt.Printf("%47s", " ")
	for _, element := range "Sub Total: RWF" {
...
func printFinalBill() {
	...
	fmt.Printf("%s\n", strings.Repeat("*", 91))
	time.Sleep(time.Millisecond * 200)
	fmt.Printf("%86s\n", "Kigali, Makuze Peace Plaza, First Floor, Foodie")
	time.Sleep(time.Millisecond * 200)
	fmt.Printf("%50s\n", "Tel: 250793082404")
	...
	tax := 0.0
	grandTotal := subTotalBill

	time.Sleep(time.Millisecond * 200)
	fmt.Printf("%71s: RWF%.2f\n", "TAX", tax) //display tax amount
	fmt.Printf("+%s+\n", strings.Repeat("-", 90))
	time.Sleep(time.Millisecond * 200)
